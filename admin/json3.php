...
$result = mysql_query('SELECT COUNT(aficha.ALUFIC) FROM aficha INNER JOIN citas ON (aficha.ALUFIC = citas.ALUFIC)'.$where."AND citas.status = 3");
...
$result = mysql_query("SELECT aficha.ALUFIC, aficha.ALUAPP, aficha.ALUAPM, aficha.ALUNOM, citas.fecha  FROM aficha INNER JOIN citas ON (aficha.ALUFIC = citas.ALUFIC) $where AND citas.status = 3 $sort $limit");
